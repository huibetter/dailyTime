...
- 浏览器开发预览使用 `localStorage` 持久化项目、文档和界面设置，首次启动为空工作空间。
- Tauri 桌面运行时使用 SQLite 保存项目、文档和本机用户配置，首次启动只创建空的默认工作空间。
- 从旧版本升级时，桌面端会将已有的浏览器 `localStorage` 本机配置迁移到 SQLite；已有桌面项目和文档不会被自动删除。
